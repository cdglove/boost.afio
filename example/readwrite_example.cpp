...
int main(void)
{
  try
  {
    ...
    afio::async_path_op_req req("example_file.txt", afio::file_flags::Create |
                                                    afio::file_flags::ReadWrite);
    afio::async_io_op openfile(
    ...
    afio::async_io_op resizedfile(dispatcher->truncate(
    openfile,
    ...
    afio::async_io_op written(dispatcher->write(afio::make_async_data_op_req(
    ...
    afio::async_io_op written2(dispatcher->write(afio::make_async_data_op_req(
    written, buffers2, 0))); /*< schedules write after previous write completes >*/

    // Schedule making sure the previous batch has definitely reached physical
    // storage
    // This won't complete until the write is on disc
    afio::async_io_op stored(
    dispatcher->sync(written2)); /*< schedules sync after write completes >*/
    ...
    afio::async_io_op read(dispatcher->read(afio::make_async_data_op_req(
    ...
    afio::when_all({openfile, resizedfile, written, written2, stored, read})
    ...
    afio::when_all({req.precondition /*close*/,
                    deletedfile}).get(); /*< waits for file close and delete to
