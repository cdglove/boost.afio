...
namespace
{
                          ...
                          size_t chunk_size = 1024 * 1024 /* 1Mb */)
  {
    ...
    for(auto &ih : ihs)
    {
      ...
      offset, bytes, detail::make_unique<file_buffer_type>(chunk_size)));
      offset += bytes;
    }
    ...
    for(int idx = 0; idx < (int) ihs_reqs.size(); idx++)
    {
      ...
      for(off_t o = 0; o < bytes; o += chunk_size)
      {
        ...
        auto readchunk =
        dispatcher->read(make_async_data_op_req(last, buffer->data(), thischunk, o));
        // Schedule a writing of buffer to offset offset+o after readchunk is ready
        auto writechunk = dispatcher->write(
        make_async_data_op_req(readchunk, buffer->data(), thischunk, offset + o));
